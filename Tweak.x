#import <Foundation/Foundation.h>
#import <UIKit/UIKit.h>

static void BHFLog(NSString *format, ...)
{
    va_list args;
    va_start(args, format);

    NSString *message =
        [[NSString alloc] initWithFormat:format arguments:args];

    va_end(args);

    NSLog(@"[BumbleHeatFix] %@", message);
}

static UIWindow *BHFGetWindow(void)
{
    UIApplication *app = [UIApplication sharedApplication];

    if (@available(iOS 13.0, *)) {
        for (UIScene *scene in app.connectedScenes) {

            if (![scene isKindOfClass:[UIWindowScene class]]) {
                continue;
            }

            UIWindowScene *windowScene = (UIWindowScene *)scene;

            if (windowScene.activationState != UISceneActivationStateForegroundActive) {
                continue;
            }

            for (UIWindow *window in windowScene.windows) {
                if (window.isKeyWindow) {
                    return window;
                }
            }

            if (windowScene.windows.count > 0) {
                return windowScene.windows.firstObject;
            }
        }
    }

    return nil;
}

static void BHFShowOverlay(void)
{
    dispatch_async(dispatch_get_main_queue(), ^{
        
        UIWindow *window = BHFGetWindow();

        if (!window) {
            BHFLog(@"ERROR: Could not find Bumble window");
            return;
        }

        BHFLog(@"Found window: %@", window);

        UIView *oldView = [window viewWithTag:987654];
        [oldView removeFromSuperview];

        UIView *panel = [[UIView alloc] initWithFrame:CGRectMake(15, 55, 260, 110)];

        panel.tag = 987654;
        panel.backgroundColor =
            [[UIColor blackColor] colorWithAlphaComponent:0.85];

        panel.layer.cornerRadius = 12.0;
        panel.layer.masksToBounds = YES;

        UILabel *label =
            [[UILabel alloc] initWithFrame:CGRectMake(12, 10, 236, 90)];

        label.numberOfLines = 0;
        label.textColor = UIColor.whiteColor;
        label.font = [UIFont systemFontOfSize:14.0 weight:UIFontWeightMedium];

        label.text =
            @"BumbleHeatFix\n"
             "DIAGNOSTIC MODE\n"
             "Dylib loaded successfully";

        [panel addSubview:label];
        [window addSubview:panel];

        BHFLog(@"Diagnostic overlay displayed");
    });
}

%ctor
{
    @autoreleasepool {

        BHFLog(@"================================");
        BHFLog(@"BumbleHeatFix dylib loaded");
        BHFLog(@"Process: %@", [[NSProcessInfo processInfo] processName]);
        BHFLog(@"iOS: %@", [[UIDevice currentDevice] systemVersion]);
        BHFLog(@"================================");

        dispatch_after(
            dispatch_time(DISPATCH_TIME_NOW, 2 * NSEC_PER_SEC),
            dispatch_get_main_queue(),
            ^{
                BHFLog(@"2-second checkpoint");
                BHFShowOverlay();
            }
        );
    }
}
